...
type GetExpensesBetweenUsersReturn = {
    expenses: any[],
    settlements: any[],
    balance: number,
    otherUser: {
...
type CreateExpenseReturn = Id<"expenses">;
...
export const createExpense = mutation({
    ...
    handler: async (ctx, args): Promise<CreateExpenseReturn> => {
...
export const getExpensesBetweenUsers = query({
    ...
    handler: async (ctx, { userId }): Promise<GetExpensesBetweenUsersReturn> => {
        ...
        const theirPaid = await ctx.db.query("expenses")
        ...
        const expenses = candidateExpenses.filter((e) => {
            const meInSplit = e.splits.some((s) => s.userId === me._id);
            const themInSplit = e.splits.some((s) => s.userId === userId);

            const meInvolved = e.paidByUserId === me._id || meInSplit;
            const themInvolved = e.paidByUserId === userId || themInSplit;
...
export const deleteExpense = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const relatedSettlements = allSettlements.filter(
            (settlement) => settlement.relatedExpenseIds !== undefined &&
