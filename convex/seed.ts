...
async function createGroups(ctx: any, users: any[]): Promise<any[]> {
  const now = Date.now();
...
async function createOneOnOneExpenses(ctx: any, users: any[]): Promise<any[]> {
  const now = Date.now();
...
async function createGroupExpenses(ctx: any, users: any[], groups: any[]): Promise<any[]> {
  const now = Date.now();
  const oneWeekAgo = now - 7 * 24 * 60 * 60 * 1000;
  const twoWeeksAgo = now - 14 * 24 * 60 * 60 * 1000;
...
async function createSettlements(
  ...
  return await Promise.all(
    settlementIds.map(async (id: any) => {
